...
export default function ControllersPage() {
  ...
  const [controllers, setControllers] = useState<ControllerItem[]>([]);
  ...
  useEffect(() => {
    const fetchControllers = async () => {
      try {
        ...
        // Transform the grouped data into a flat array with type information
        const controllersList: ControllerItem[] = [];
        Object.entries(data).forEach(([type, names]) => {
          if (names && Array.isArray(names)) {
            names.forEach(name => {
              controllersList.push({ name, type });
            });
          }
        });
  ...
  // Filter controllers based on search term (fuzzy search) and sort alphabetically
  const filteredControllers = useMemo(() => {
    let filtered = controllers;

    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      filtered = controllers.filter((controller) => {
        const nameLower = controller.name.toLowerCase();
        const typeLower = controller.type.toLowerCase();
  ...
  const handleControllerClick = (controller: ControllerItem) => {
    router.push(`/controllers/${controller.type}/${controller.name}`);
  };
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Explore available trading controllers and their configurations
              </p>
            </div>
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          {paginatedControllers.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {searchTerm ? 'No controllers match your search.' : 'No controllers available.'}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {paginatedControllers.map((controller) => (
                <Card
                  key={`${controller.type}-${controller.name}`}
                  ...
                  <CardContent className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-medium truncate">{controller.name}</h3>
                        <p className="text-sm text-muted-foreground truncate">
                          {formatType(controller.type)}
                        </p>
                      </div>
                    </div>
